/**
 * DPK Kota Sawahlunto — Core Client Script (NO SHADOWS)
...
/* ==========================================================================
   2. Modal Management (Profil & Unduh, Zero Shadows)
   ========================================================================== */
function initModals() {
...
/* ==========================================================================
   3. Unduhan List Population (Flat, No Shadows)
   ========================================================================== */
  ...
  container.innerHTML = unduhFilesData.map(file => `
    <div class="bg-white/5 hover:bg-white/10 p-4 rounded-2xl border border-white/10 transition-colors flex items-center justify-between gap-4">
      <div class="space-y-1">
        ...
        <div class="flex items-center gap-2 text-xs text-[#D4A373]">
          <span class="px-2 py-0.5 rounded bg-white/10 text-white font-mono border border-white/10">${file.format}</span>
          <span>${file.size}</span>
          <span>&bull;</span>
          <span class="text-gray-400">${file.date}</span>
        </div>
      </div>
      <button onclick="handleDownloadSim('${file.title}', '${file.filename}')" class="px-4 py-2 rounded-full bg-[#C59B27] hover:bg-[#B3891E] text-white text-xs font-medium cursor-pointer transition flex items-center gap-1.5 shrink-0 border border-[#C59B27]">
        <span>Unduh</span>
...
window.handleDownloadSim = function(title, filename) {
  // Graceful simulation notification without alerts or emojis, flat border
  ...
  toast.textContent = `Mengunduh berkas resmi DPK: ${filename}`;
