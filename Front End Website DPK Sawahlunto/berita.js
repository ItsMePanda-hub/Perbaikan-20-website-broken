/* ================================================================
   berita.js — Logika Halaman Warta Pustaka DPK Sawahlunto
   (Direct Link to Full Page, Journalistic Card Layout, Zero Emojis, Zero Shadows)
   ================================================================ */
...
(function () {
  ...
  // 2. Parse URL parameters (e.g. ?category=Perpustakaan+Adinegoro or ?q=search)
  ...
  // 3. Render Featured Headline Story (Flat)
      ...
      <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 items-center">
        ...
        <div class="lg:col-span-7 space-y-4">
          <div class="flex items-center gap-3">
            <span class="inline-block px-3.5 py-1 rounded-full bg-[#C59B27] text-white text-xs font-semibold uppercase tracking-wider">
              Sorotan Utama &bull; ${featured.category}
            </span>
            <span class="text-xs text-[#EFECE2]">
          ...
          <h3 class="font-primary font-bold text-2xl sm:text-4xl lg:text-[40px] leading-tight text-white">
            <a href="berita-detail.html?id=${featured.id}" class="hover:text-[#C59B27] transition">
              ${featured.title}
            </a>
          </h3>

          <p class="text-sm sm:text-base text-[#EFECE2] font-secondary leading-relaxed line-clamp-3">
            ${featured.excerpt}
          </p>

          <div class="pt-3 flex items-center justify-between gap-4">
            <span class="text-xs text-[#C59B27] font-medium">Penulis: ${featured.author}</span>
            <a href="berita-detail.html?id=${featured.id}" class="btn-pill-gold px-7 py-2.5 text-sm font-medium inline-block">
              Baca Warta Lengkap
            </a>
          </div>
        </div>
        ...
        <div class="lg:col-span-5">
          <div class="rounded-2xl overflow-hidden aspect-video lg:aspect-square border border-white/20">
            <img src="${featured.image || 'assets/hero-dpk.jpg'}" alt="${featured.title}" class="w-full h-full object-cover">
          </div>
        </div>
  ...
  // 6. Render News Grid (Journal Cards, Flat, No Shadows)
  function renderNewsGrid(articles) {
    ...
    if (articles.length === 0) {
      ...
      if (newsCountLabel) newsCountLabel.textContent = '0 Warta Ditemukan';
      ...
      return `
        <article class="journal-card">
          <div class="journal-card-image-wrap">
            <img src="${article.image || 'assets/hero-dpk.jpg'}" alt="${article.title}">
          ...
          <div class="p-6 sm:p-7 flex flex-col justify-between flex-1">
            <div>
              <span class="text-xs text-[#415A77] block mb-2">${article.date}</span>
            ...
            <div class="pt-6 border-t border-[#0D1B2A]/10 mt-6 flex items-center justify-between">
              <span class="text-xs text-[#415A77]">${article.readTime || '3 menit baca'}</span>
              <a href="berita-detail.html?id=${article.id}" class="btn-pill-ink px-5 py-2 text-xs font-medium inline-block">
                Selengkapnya
              </a>
            </div>
          </div>
  ...
  // 7. Render Pagination Controls (Flat)
      ...
      buttons += `
        <button 
          ...
          class="w-10 h-10 rounded-full font-medium text-sm transition ${
            isActive
              ? 'bg-[#0D1B2A] text-white font-bold border border-[#0D1B2A]'
              : 'bg-white hover:bg-[#F7F5EE] text-[#0D1B2A] border border-[#0D1B2A]/20'
          }"
        >
          ${i}
        </button>
